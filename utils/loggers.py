...
class CsvLogger:
    def __init__(self, setting_str: str, dataset_str: str,
                 model_str: str, grid_until=None) -> None:
        self.accs = []
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        path = base_path() + "results/" + self.setting + "/" + self.dataset\
               + "/" + self.model + ("/mean_accs.csv" if self.grid_until is None else "/mean_grid_%03d.csv" % self.grid_until)
