...
class SequentialCIFAR10(ContinualDataset):
    ...
    def get_data_loaders(self):
        ...
        while not ((os.path.isdir(base_path() + 'CIFAR10') and os.path.isfile(base_path() + 'CIFAR10_DONE'))
                   or not os.path.isdir(base_path() + 'CIFAR10')):
            sleep(1)
        ...
        with open(base_path() + 'CIFAR10_DONE', 'w') as f:
            f.write('we')

        if self.args.lpc is not None:
            return self.prepare_data_loaders(train_dataset, test_dataset)
        else:
